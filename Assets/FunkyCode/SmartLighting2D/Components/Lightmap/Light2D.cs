...
namespace FunkyCode {
    ...
    public class Light2D : LightingMonoBehaviour {
        ...
        public bool InCameras() {
            ...
            return lightingCameras.Any(t => t.Camera != null && t.WorldRect().Overlaps(lightRect));
        }
        ...
        public bool IfDrawLightCollider(LightCollider2D lightCollider) {
            ...
            return layerSetting.Any(t => t != null && IsMatchLayer(t, lightCollider));
        }

        private bool IsMatchLayer(LayerSetting layerSetting, LightCollider2D lightCollider) {
            int layerID = layerSetting.layerID;
            LightLayerType lightLayerType = layerSetting.type;
            return
                lightLayerType == LightLayerType.ShadowAndMask ? layerID == lightCollider.shadowLayer || layerID == lightCollider.maskLayer :
                lightLayerType == LightLayerType.MaskOnly ? layerID == lightCollider.maskLayer :
                lightLayerType == LightLayerType.ShadowOnly ? layerID == lightCollider.shadowLayer :
                false;
        }
